...
func NewVolumeEntryFromRequest(req *api.VolumeCreateRequest) *VolumeEntry {
	...
	case durability == api.DurabilityDistributeOnly || durability == "":
		println("[%v] Distributed", vol.Info.Id)
...
func (v *VolumeEntry) Create(db *bolt.DB,
	...
	if len(possibleClusters) == 0 {
		println("Volume being ask to be created, but there are no clusters configured")
		return NoSpace
	}
	println("Using the following clusters: %+v", possibleClusters)
	...
	for _, cluster := range clusters {
		...
		if err == nil {
			...
			println("Volume to be created on cluster %v", cluster)
			break
		} else if err == NoSpace ||
			err == MaxBricks ||
			err == MinimumBrickSize {
			println("Cluster %v can not accommodate volume "+
				"(%v), trying next cluster", cluster, err)
			continue
		} else {
			// A genuine error occurred - bail out
			println("Error calling v.allocBricksInCluster: %v", err)
		}
	}
...
func (v *VolumeEntry) Destroy(db *bolt.DB, executor executors.Executor) error {
	println("Destroying volume %v", v.Info.Id)
	...
	db.View(func(tx *bolt.Tx) error {
		for index, id := range v.BricksIds() {
			...
			if err != nil {
				println("Brick %v not found in db: %v", id, err)
				continue
			}
			...
			if sshhost == "" {
				...
				if err != nil {
					println("Unable to determine brick node: %v", err)
					return err
				}
	...
	if err != nil {
		println("Unable to delete volume: %v", err)
		return err
	}
	...
	if err != nil {
		println("Unable to delete bricks: %v", err)
		return err
	}
...
func (v *VolumeEntry) Expand(db *bolt.DB,
	...
	defer func() {
		if e != nil {
			println("Error detected, cleaning up")
