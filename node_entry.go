...
func GetVerifiedManageHostname(db *bolt.DB, e executors.Executor, clusterId string) (string, error) {
	...
	for _, n := range cluster.Info.Nodes {
		...
		if err != nil {
			println("Glusterd not running in %v", newNode.ManageHostName())
			continue
		}
		node = newNode
		break
			...
			for _, id := range n.Devices {
				...
				if err != nil {
					if err == NoReplacement {
						println("Unable to remove node [%v] as no device was found to replace device [%v]", n.Info.Id, d.Id())
					}
					return err
				}
			}
