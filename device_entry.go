...
func (d *DeviceEntry) SetState(db *bolt.DB,
	...
	case api.EntryStateOffline:
		...
		case api.EntryStateFailed:
			...
			err := d.Remove(db, e, a)
			if err != nil {
				if err == NoReplacement {
					println("Unable to delete device [%v] as no device was found to replace it", d.Id())
					return err
				}
				return err
			}
...
func (d *DeviceEntry) NewBrickEntry(amount uint64, snapFactor float64, gid int64, volumeid string) *BrickEntry {
	...
	println("device %v[%v] > required size [%v] ?",
		d.Id(),
...
func (d *DeviceEntry) Remove(db *bolt.DB,
	...
	for _, brickId := range d.Bricks {
		...
		println("Replacing brick %v on device %v on node %v", brickEntry.Id(), d.Id(), d.NodeId)
		err = volumeEntry.replaceBrickInVolume(db, executor, allocator, brickEntry.Id())
		if err != nil {
			println(fmt.Errorf("Failed to remove device, error: %v", err))
			return err
		}
	}
